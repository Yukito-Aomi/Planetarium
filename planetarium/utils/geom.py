...
def create_geom(parent, vertices: list, color: tuple[float, float, float], thickness: float = DEFAULT_THICKNESS):
    """
    図形オブジェクトを生成するヘルパー関数
...
def draw_line(parent, point1, point2, color: tuple[float, float, float], thickness: float = DEFAULT_THICKNESS):
    """
    直線を描画する関数
    :param parent: スーパーノード
    :param point1: 直線の端点１
...
def draw_circumference_zx(parent, radius, color: tuple[float, float, float], thickness: float = DEFAULT_THICKNESS, segments: int = DEFAULT_SEGMENTS):
    """
    ＺＸ平面上に円を描画する関数
    :param parent: スーパーノード
    :param radius: 円の半径
...
def draw_filled_triangle(parent, point1, point2, point3, color: tuple[float, float, float]):
    """
    面が塗りつぶされた三角形を描画する関数
    :param parent: スーパーノード
